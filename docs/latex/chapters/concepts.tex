...
\begin{subsecao}{A arquitetura de Software}\label{subsec:arquitetura}
  ...
  \emph{stakeholders} a entenderem e analisarem como o sistema irá alcançar características 
  ...
  inconsistente e alta incidência de \emph{bugs}. A combinação dos dois fatores anteriores 
...
\begin{subsecao}{Arquitetura Limpa} 
  ...
  (\emph{Separation of Concerns}) \cite[p.202]{clean_arch}. Esta consiste em segregar todo o código em módulos de 
  ...
  por tratar as requisições recebidas pelos \emph{Frameworks} a fim de executar os casos de uso 
  para gerenciar os usuários.}
  \paragrafo{Por fim, os \emph{Frameworks} e \emph{Drivers} são geralmente, no contexto de 
  ...
  testáveis, independentes de \emph{frameworks}, \emph{\acs{UI}}, banco de dados ou qualquer agência externa.}
  \end{subsecao}
  \end{secao}
  ...
  \begin{secao}{SOLID} \label{sec:solid}
    ...
    \paragrafo{Também proposto por Robert C. Martin \cite[58]{clean_arch}, SOLID é um acrônimo para siglas que 
    ...
    \begin{subsecao}{\emph{Liskov Substitution Principle}} \label{subsec:LSP}
      \paragrafo{Segundo o Princípio da Subsituição de \emph{Liskov}, as classes devem ser capazes 
...
\begin{secao}{Design Patterns} \label{sec:patterns}
  ...
  \paragrafo{Como alguns princípios vistos na seção de \emph{Clean Architecture}, os \emph{Design Patterns} tem como principal função garantir a reusabilidade do código com baixo 
  ...
  \begin{lista}
   \itemlista{\emph{Abstract Factory}}
   \itemlista{\emph{Builder}}
   \itemlista{\emph{Factory Method}}
   \itemlista{\emph{Prototype}}
   \itemlista{\emph{Singleton}}
  \end{lista}

  \paragrafo{Padrões Estruturais}
  \begin{lista}
    \itemlista{\emph{Adapter}}
    \itemlista{\emph{Bridge}}
    \itemlista{\emph{Composite}}
    \itemlista{\emph{Decorator}}
    \itemlista{\emph{Facade}}
    \itemlista{\emph{Flyweight}}
    \itemlista{\emph{Proxy}}
  \end{lista}
  
  \paragrafo{Padrões Comportamentais}
  \begin{lista}
    \itemlista{\emph{Chain of Responsiblity}}
    \itemlista{\emph{Command}}
    \itemlista{\emph{Interpreter}}
    \itemlista{\emph{Iterator}}
    \itemlista{\emph{Mediator}}
    \itemlista{\emph{Memento}}
    \itemlista{\emph{Observer}}
    \itemlista{\emph{State}}
    \itemlista{\emph{Strategy}}
    \itemlista{\emph{Template Method}}
